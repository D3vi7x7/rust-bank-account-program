...
impl BankAccount {
    ...
    fn deposit(mut self, amount: f64) -> BankAccount {
    ...
    fn withdraw(mut self, amount: f64) -> BankAccount {
    ...
    fn transfer(mut self,mut to_account: BankAccount, amount: f64) -> (BankAccount, BankAccount) {
        if(amount > self.balance){
            println!("INSUFFICIENT BALANCE !!!");
            (self,to_account)
        }else{
